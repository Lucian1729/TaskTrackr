...
class Node():
    def __init__(self, fellow, my_ip):
        ...
        self.mydb = mysql.connector.connect(host="localhost", user="<your-username>", password="<your-password>", database="task_management")
        self.log = []
        self.staged = None
        self.term = 0
    ...
    def ask_for_vote(self, voter, term):
        ...
        while self.status == CANDIDATE and self.term == term:
            reply = send(voter, route, message)
            if reply:
    ...
    def update_follower_commitIdx(self, follower):
        ...
        if reply and reply.json()["commitIdx"] < self.commitIdx:
            reply = send(follower, route, second_message)
    ...
    def send_heartbeat(self, follower):
        ...
        while self.status == LEADER:
            start = time.time()
            reply = send(follower, route, message)
            if reply:
